...
*   [ ] **6. 模块：班级管理 (Class)**
    *   **后端:**
        *   [ ] 创建 `Clazz` 实体类, `ClazzQuery` (注意 Class 是关键字，用 Clazz 或其他名称)。
        *   [ ] 创建 `ClazzMapper` 接口及 XML。
        *   [ ] 创建 `ClazzService` 接口及实现 (CRUD, 导入, 导出, 自动计算结束日期, 自动判断状态)。
        *   [ ] 创建 `ClazzController`。
        *   [ ] 编写测试。
    *   **前端:**
        *   [ ] 创建 `ClassManagement.vue` 页面。
        *   [ ] 实现班级信息的展示、增删改查、导入导出。
        *   [ ] 班主任字段使用下拉选择（调用教师列表接口）。
